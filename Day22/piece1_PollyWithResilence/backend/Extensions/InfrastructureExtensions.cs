...
#pragma warning restore EXTEXP0018
        ...
        // Wraps an outbound HTTP dependency (ExternalQuoteClient → /api/stub/service)
        // with four defensive strategies, executed outer → inner on every call:
        //
        //  [1] Bulkhead   — ConcurrencyLimiter: max 5 concurrent, 5 queued, reject the rest
        //  [2] CircuitBreaker — opens after ≥60% failures in a 10-s window (min 5 calls);
        //                       stays open 5 s, then probes once (half-open)
        //  [3] Retry      — up to 2 retries with exponential back-off (300 ms base, jitter);
        //                   only applied to idempotent GET calls (this client is GET-only)
        //  [4] Timeout    — 2-second per-attempt deadline (innermost, closest to the call)
        services.AddSingleton<FaultSwitch>();
        services.AddSingleton<CircuitBreakerStateTracker>();

        var externalBaseUrl = configuration["ExternalService:BaseUrl"] ?? "http://localhost:5000";
        ...
        services
            ...
            .AddResilienceHandler("external-quotes", (builder, ctx) =>
            {
                ...
                // ── [1] Bulkhead ───────────────────────────────────────────────
                // Reject excess concurrent calls before they can stress the CB.
                ...
                // ── [2] Circuit Breaker ────────────────────────────────────────
                // Uses HttpCircuitBreakerStrategyOptions which pre-wires ShouldHandle
                // to cover HttpRequestException, 5xx responses, and 408 timeouts.
                builder.AddCircuitBreaker(new HttpCircuitBreakerStrategyOptions
                {
                    ...
                    OnOpened = args =>
                    {
                        var trigger = args.Outcome.Exception?.GetType().Name
                            ?? args.Outcome.Result?.StatusCode.ToString()
                            ?? "unknown";
                        log.LogWarning(
                            "[CB] OPENED — breaking for {Dur:N1}s | trigger={Trigger}",
                            args.BreakDuration.TotalSeconds, trigger);
                        tracker.LogEvent("CB-OPENED",
                            $"break={args.BreakDuration.TotalSeconds:N1}s trigger={trigger}");
                        return ValueTask.CompletedTask;
                    },

                    OnHalfOpened = _ =>
                    {
                        log.LogInformation("[CB] HALF-OPEN — one probe request allowed");
                        tracker.LogEvent("CB-HALF-OPEN", "probe request allowed");
                        return ValueTask.CompletedTask;
                    },

                    OnClosed = _ =>
                    {
                        log.LogInformation("[CB] CLOSED — circuit recovered");
                        tracker.LogEvent("CB-CLOSED", "circuit recovered");
                        return ValueTask.CompletedTask;
                    }
                });

                // ── [3] Retry with exponential back-off ───────────────────────
                // Safe because ExternalQuoteClient exclusively issues GET requests.
                builder.AddRetry(new HttpRetryStrategyOptions
                {
                    ...
                    {
                        log.LogWarning(
                            "[Retry] attempt={Att} | delay={Delay}ms",
